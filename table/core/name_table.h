...
class NameTable : public SubTableContainerTable, public RefCounted<NameTable> {
 public:
  ...
  class NameEntry : public RefCounted<NameEntry> {
   public:
    ...
    // Returns the name in Unicode as wchar_t array.
    // Note: ICU wchar_t* convention requires caller to delete[] it.
    wchar_t* Name();
  ...
  // Get the name as a wchar_t* for the given name record. If there is no
  // encoding conversion available for the name record then a best attempt
  // wchar_t* will be returned.
  // Note: ICU wchar_t* convention requires caller to delete[] it.
  virtual wchar_t* Name(int32_t index);

  // Get the name as a wchar_t* for the specified name. If there is no entry for
  // the requested name then NULL is returned. If there is no encoding
  // conversion available for the name then a best attempt wchar_t* will be
  // returned.
  // Note: ICU wchar_t* convention requires caller to delete[] it.
 ...
 private:
  ...
  // Note: ICU wchar_t* convention requires caller to delete[] it.
  static wchar_t* ConvertFromNameBytes(std::vector<uint8_t>* name_bytes,
